...
def _llm_reply(db: Client, message: str, history: list[dict]) -> dict:
    ...
    system = (
        ...
        "treat each date as a lesson to CREATE unless the note explicitly says 取消(cancelled) "
        "or 改期(rescheduled) without giving a replacement date — skip those.\n"
        "  - 改為 = rescheduled TO a specific date: create the lesson on the NEW date, skip the old one.\n"
        "  - 取消 = cancelled: skip that date entirely.\n"
        "  - If a date has a note in parentheses but no clear instruction, still create that lesson.\n\n"
