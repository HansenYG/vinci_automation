-- Update lesson_schedule view to compute 'hasacceptance' status when
-- the lesson's raw status is 'HasAcceptance'. This makes the Dashboard
-- bucket-2 filter (has_acceptance) actually return lessons, so the
-- admin sees "Assign" buttons for lessons with accepted offers.
